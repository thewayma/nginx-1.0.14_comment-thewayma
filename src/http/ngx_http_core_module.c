...
static ngx_command_t  ngx_http_core_commands[] = {
    ...
    { ngx_string("listen"),
      NGX_HTTP_SRV_CONF/*隶属于srv_conf*/|NGX_CONF_1MORE,
    ...
    { ngx_string("server_name"),
      ...
      ngx_http_core_server_name/*虚拟主机支持, 原理: 多级哈希表 支持 全匹配, 前置与后置通配符*/,
      NGX_HTTP_SRV_CONF_OFFSET,
      0,
      NULL },
...
static ngx_http_module_t  ngx_http_core_module_ctx = {
    ngx_http_core_preconfiguration,        /* preconfiguration - ngx_http_variables_add_core_vars 初始化 内置动态变量*/
...
/**
 * 1. http 11 phase handler处理框架初始化: ngx_http_block -> ngx_http_init_phase_handlers
...
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    if (rc == NGX_OK) {         //!< 这个phase OK, 进入下个phase
        r->phase_handler = ph->next;
        return NGX_AGAIN;
    }

    if (rc == NGX_DECLINED) {   //!< NGX_DECLINED: 过此phase中的 下个handler
        r->phase_handler++;
        return NGX_AGAIN;
    }
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    /**
     * 1. 根据http request uri 在static location tree查找匹配的 location configuration
     * 2. 根据http request uri 在正则匹配查找对应的 location configuration
     */
    rc = ngx_http_core_find_location(r);

    if (rc == NGX_ERROR) {                                              //!< 没找到对应location
        ngx_http_finalize_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);   //!< 回收request，然后返回OK，也就是停止handler的处理  
        return NGX_OK;
    }

    /**
     * 经过static location tree, or 正则查找后, 获取对应的ngx_http_core_loc_conf_t 首地址
     */
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    rc = ph->handler(r);    //!< call ngx_http_static_handler
...
ngx_http_core_find_location(ngx_http_request_t *r)
{
    ...
    /**
     * 完全匹配成功, 返回NGX_OK
     * 内部跳转(auto redirect)成功, 返回NGX_DONE
     * 前缀匹配(优先前缀匹配, 普通前缀匹配)成功返回NGX_AGAIN
     */
    rc = ngx_http_core_find_static_location(r, pclcf->static_locations/*server{} 对应的三叉树 树根*/);    //!< 根据http request uri 在static location tree查找匹配的 location configuration
    ...
    if (rc == NGX_AGAIN) {
        ...
        noregex = clcf->noregex;    //!< 优先匹配 ^~ xxx, clcf->norege设置为1
#endif
...
#if (NGX_PCRE)
    if (noregex == 0/*优先匹配 ^~ xxx, clcf->norege设置为1*/ && pclcf->regex_locations) {
...
/**
 * NGX_OK       - exact match           exact match 精确匹配(=)
 * NGX_DONE     - auto redirect         auto redirect 表示location结尾是“/”符号, 比如“/a/ {}” 但请求是“/a”
 * NGX_AGAIN    - inclusive match       inclusive match 一般匹配(^~(优先前缀匹配) | 无修饰符(普通前缀匹配))
 * NGX_DECLINED - no match              no match 没有匹配成功,这里的没有匹配成功表示没有在子级匹配成功
 *
 ...
 *
 * 可以从下面的代码看出，三叉树优化了static location的查找过程，防止了O(n)的复杂度来匹配location
 */
static ngx_int_t
ngx_http_core_find_static_location(ngx_http_request_t *r, ngx_http_location_tree_node_t *node/*三叉树根*/)
{
    u_char     *uri;
    size_t      len, n;
    ngx_int_t   rc, rv;
    ...
    for ( ;; ) {
        ...
        n = (len <= (size_t) node->len) ? len : node->len;  //!< n是uri的长度和node name长度的最小值，好比较他们的交集

        rc = ngx_filename_cmp(uri, node->name, n);          //!< 比较uri和node 的name交集

        if (rc != 0) {                                      //!< 不得0表示uri和node的name不相等，这时候三叉树就能加速查找的效率，选择node的左节点或者右节点
            node = (rc < 0) ? node->left : node->right;

            continue;                                       //!< !!!!!!!!!!!!!!!!!!  uri 与 node->name 共同部分(0-len) 不同, 则跳向left or right !!!!!!!!!!!!!!!!!!!!!!!
        }

        /**
         * 以下则是 uri 与 node->name 共同部分(0-len), 相同情况
         *
         *  注意: 排序去重后, 各类型的location_queue_t按照以下顺序排列
         ...
         */
        if (len > (size_t) node->len) {                     //!< 场景1. 如果交集相等，如果uri的长度比node的长度还要长

            /** 共同前缀匹配
             * 场景: /abc <==> =/abcd <===> /abcd, 输入uri为/abcd
             * 则生成的location tree为: 
             *                          /abc
             *                            |
             *                            | tree节点
            ...
            if (node->inclusive) {                          //!< 如果这个节点是前缀匹配的那种需要递归tree节点，因为tree节点后面的子节点拥有相同的前缀
                 ...
                 * 为什么要嵌套递归呢，因为location的嵌套配置虽然官方不推荐，但是配置的话，父子location需要有相同的前缀, 所以需要递归嵌套location
                 */
                rv = NGX_AGAIN;

                node = node->tree;                          //!< node重新变为tree节点, 即在其共同前缀节点中继续匹配
                uri += n;                                   //!< 更新uri 下次要匹配的位置
                len -= n;                                   //!< 更新uri 下次要匹配的长度

                continue;
            }

            /** exact only, 精确匹配
             * 对于精确匹配 ＝/abc 和 后续前缀匹配/abcd两个location配置，/abcd 不会是 =/abc节点的tree节点, /abcd 只能是 =/abc的right节点
             *
             * 场景: =/abc <===> /abcd, uri为/abcd
             * 则生成location tree为:
             *             =/abc
             *                  \
             *                   \ right右子树
             *                    \
             *                      /abcd
             * 当前节点node为=/abc对应的节点, 则下一步必须迭代node->right, 已找到/abcd对应的节点;
             * 因为 当前node节点为完全匹配时, node后面的节点必须是在node->right上面, 具体见ngx_http_create_locations_list
             */
            node = node->right;
        ...
        if (len == (size_t) node->len) {                    //!< 场景2. 如果是uri和node的name是完全相等的  
        ...
        if (len + 1 == (size_t) node->len && node->auto_redirect) { //!< auto redirect 表示location结尾是“/”符号, 比如“/a/ {}” 但请求是“/a”
        ...
        /** len(uri) < node->len - 1
         * 如果前缀相等，uri的长度比node的长度还要小，比如node的name是/abc ，uri是/ab,这种情况是/abc 一定是精确匹配，因为如果是前缀匹配那么／abc 肯定会在／ab的tree 指针里面
         */
        node = node->left;
    }   //!< for (;;)
}
...
ngx_http_output_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    rc = ngx_http_top_body_filter(r, in);   //!< ngx_http_copy_filter
...
/** server{} 命令块解析
 * NGX_HTTP_MAIN_CONF, 隶属于main_conf
 */
static char *
ngx_http_core_server(ngx_conf_t *cf/*cf->ctx 为ngx_http_block(main_conf) 创建的ngx_http_conf_ctx_t*/, ngx_command_t *cmd, void *dummy)   //!< 创建server {} 的配置上下文
{
    char                        *rv;
    void                        *mconf;
    ngx_uint_t                   i;
    ...
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));   //!< 分配server 对应的 配置上下文
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }

    http_ctx = cf->ctx;                     //!< main_conf 创建的 ngx_http_conf_ctx_t
    ctx->main_conf = http_ctx->main_conf;   //!< 对于server {}, main conf不变
    ...
    /**
     * 把上面的( ctx->srv_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module) ) 创建的srv_conf首地址
     * 挂入main_conf中的 ngx_http_conf_ctx_t->servers 动态数组中
     *
     * 此后, main_conf就能 遍历到 每个server{} 分配的srv_conf 配置了 
     */
    cscf = ctx->srv_conf[ngx_http_core_module.ctx_index/*初始化为0*/];  //!< server_ctx 为ngx_http_core_module(自身) 分配的ngx_http_core_srv_conf_t
    cscf->ctx = ctx;    //!< 记录当前ngx_http_core_srv_conf_t所属的ctx(ngx_http_conf_ctx_t)


    cmcf = ctx->main_conf[ngx_http_core_module.ctx_index];  //!< 新创建的ctx(ngx_http_conf_ctx_t) 会复用 main_conf!!!!!!!!!!!!!!!!!! 

	/** 在main_conf 中 会保存 所有 src_conf创建的 srv_conf的的地址
     * 保存所有的servers，可以看到是保存在main中的。这样子最后在HTTP main中就可以取到这个srv 
     */
    cscfp = ngx_array_push(&cmcf->servers); //!< 从动态数组中申请一个position, 返回值是 地址
    if (cscfp == NULL) {
        return NGX_CONF_ERROR;
    }

    *cscfp = cscf;                          //!< srv_conf首地址 存入 main_conf中的动态数组中


    /* parse inside server{} */
	//解析server{} block，可以看到设置type为srv_conf.
    pcf = *cf;
    cf->ctx = ctx;  //!< ctx 变换为srv_conf创建的ngx_http_conf_ctx_t
    cf->cmd_type = NGX_HTTP_SRV_CONF;

    /**
     * location{} 解析命令为ngx_http_core_location
     */
    ...
    *cf = pcf;
    ...
    /** 小概率事件
     * 若server命令解析过程中,没有发现listen命令(即, cscf->listen==0)
     * 则执行一个默认listen项 (listen *:80/8000)
     *
     * 关于cscf->listen赋值时机: 在listen命令解析时, 详见ngx_http_core_listen()
     */
    if (rv == NGX_CONF_OK && !cscf->listen) {
        ...
        /**
         * 将网络序的ip地址 转换成 .十分进制ip:port, e.g.  转成 1.1.1.1:80
         */
        (void) ngx_sock_ntop(&lsopt.u.sockaddr/*网络序ip地址*/, lsopt.addr/*.十分进制ip:portNum, e.g. 1.1.1.1:80*/, NGX_SOCKADDR_STRLEN, 1);

        if (ngx_http_add_listen(cf/*ngx_http_block创建的ngx_http_conf_ctx_t*/, cscf/*ctx->srv_conf[ngx_http_core_module.ctx_index]*/, &lsopt) != NGX_OK) {
            return NGX_CONF_ERROR;
        }
    }
...
/** location 命令配置解析
 * 隶属于 NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF
 */
static char *
ngx_http_core_location(ngx_conf_t *cf/*cf->ctx 为ngx_http_core_server(srv_conf) 创建的ngx_http_conf_ctx_t*/, ngx_command_t *cmd, void *dummy)
{
    char                      *rv;
    u_char                    *mod;
    size_t                     len;
    ...
    pctx = cf->ctx;
    ctx->main_conf = pctx->main_conf;   //!< loc_conf 复用parent_ctx(cf->ctx) 的 main_conf
    ctx->srv_conf = pctx->srv_conf;     //!< loc_conf 复用parent_ctx(cf->ctx) 的 srv_conf
    ...
    /**
     * clcf 为 上面ctx->loc_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module) 创建的loc_conf[0 == ngx_http_core_module.ctx_index] 数组存的第一个值 
     */
    clcf = ctx->loc_conf[ngx_http_core_module.ctx_index];
    clcf->loc_conf = ctx->loc_conf; //!< 指向所属location 块内ngx_http_conf_ctx_t 结构体中的loc_conf 指针数组

    value = cf->args->elts;

    if (cf->args->nelts == 3) { //!< (1)location = /; (2)location ~ \.php$

        len = value[1].len;
        mod = value[1].data;
        name = &value[2];

        if (len == 1 && mod[0] == '=') {                            //!< 1. 完全匹配, exact_match

            clcf->name = *name;
            clcf->exact_match = 1;

        } else if (len == 2 && mod[0] == '^' && mod[1] == '~') {    //!< 2. 优先(相对正则, 比正则匹配优先级高)前缀匹配: ^~ 开头表示uri以某个常规字符串开头，理解为匹配 url路径即可
                                                                    //!< nginx不对url做编码，因此请求为/static/20%/aa，
            clcf->name = *name;                                     //!<    可以被规则^~ /static/ /aa匹配到（注意是空格）
            clcf->noregex = 1;

        } else if (len == 1 && mod[0] == '~') {                     //!< 3. 开头表示区分大小写的正则匹配

            if (ngx_http_core_regex_location(cf, clcf, name/*正则表达式*/, 0/*区分大小写*/) != NGX_OK) {
                return NGX_CONF_ERROR;
            }

        } else if (len == 2 && mod[0] == '~' && mod[1] == '*') {    //!< 4. 开头表示不区分大小写的正则匹配

            if (ngx_http_core_regex_location(cf, clcf, name/*正则表达式*/, 1/*不区分大小写*/) != NGX_OK) {
                return NGX_CONF_ERROR;
            }
    ...
    } else {    //!< location =xxxxx; location ~*xxxxx;
    ...
    /**
     * parent_ctx(srv_conf) loc_conf 数组的第一个值, 指向pctx->loc_conf[ngx_http_core_module.ctx_index]存放是 server(parent_ctx) 对应的 ngx_http_core_loc_conf_t的地址
     */
    pclcf = pctx->loc_conf[ngx_http_core_module.ctx_index];


    /** 小概率事件 - 嵌套location{location{}}, 哪个sb会这么配啊
     *
     * 正常情况下(server{location{}}), pclcf为外层server{} 对应的ngx_http_core_loc_conf_t地址
     * 异常情况下(location{location{}}), pclcf为外层location{} 对应的ngx_http_core_loc_conf_t地址
     *
     * 此时, 发生了location嵌套(location{location{}}) , 当前函数则是 走到内层location{}对应的ngx_http_core_location解析命令, 发现外层已经配了location(pclcf->name.len != 0)
     */
    if (pclcf->name.len) {
    ...
    /** #############################################################################################################################################################################
     * 1. 正常情况下(server{location{}}), 将当前loc_conf 链入到 parent_ctx(srv_conf) 对应的ngx_http_core_loc_conf_t->(ngx_queue_t*)locations 双向链表表尾
     * 2. 非正常情况下(location{location{}}, location发生嵌套), 将当前loc_conf 链入到 parent_ctx(外层location{}) 对应的ngx_http_core_loc_conf_t->(ngx_queue_t*)location 双向链表表尾
     * ##############################################################################################################################################################################
     */
    if (ngx_http_add_location(cf, &pclcf->locations/*ngx_queue_t **locations*/, clcf/*新分配的ctx->loc_conf[ngx_http_core_module.ctx_index]*/) != NGX_OK) {
        return NGX_CONF_ERROR;
    }
...
static ngx_int_t ngx_http_core_regex_location(ngx_conf_t *cf, ngx_http_core_loc_conf_t *clcf,
    ngx_str_t *regex/*正则表达式*/, ngx_uint_t caseless/*0:区分大小写, 1:不区分大小写*/)
{
#if (NGX_PCRE)
...
#if (NGX_HAVE_CASELESS_FILESYSTEM)  //!< 不区分大小写的fs有哪些?
...
/** listen命令配置解析
 * NGX_HTTP_SRV_CONF, 隶属于srv_conf
 */
static char *
ngx_http_core_listen(ngx_conf_t *cf/*cf->ctx 为ngx_http_core_server(srv_conf) 创建的ngx_http_conf_ctx_t*/, ngx_command_t *cmd, void *conf/*srv_conf时创建的 ngx_http_core_srv_conf_t*/)
{
    ...
    cscf->listen = 1;   //!< 标示 在server { 已出现了listen命令 }
    ...
    ngx_memzero(&lsopt, sizeof(ngx_http_listen_opt_t));     //!< listen 选项描述符

    ngx_memcpy(&lsopt.u.sockaddr, u.sockaddr, u.socklen);   //!< socket: ip + port
    ...
    for (n = 2; n < cf->args->nelts; n++) {
        ...
        {
            lsopt.default_server = 1;   //!< 标示为 default_server
            continue;
        }
    ...
    /**
     * 向 ngx_http_block创建的 ngx_http_core_main_conf_t->ports (ngx_array_t) 中插入 lsopt对应的 端口描述符(ngx_http_conf_port_t) 与 地址描述符(ngx_http_conf_addr_t)
     */
    if (ngx_http_add_listen(cf/*cf-ctx 为ngx_http_core_server(srv_conf) 创建的ngx_http_conf_ctx_t*/, cscf/*srv_conf时创建的 ngx_http_core_srv_conf_t*/, &lsopt) == NGX_OK) {
        return NGX_CONF_OK;
    }
...
/** server_name 配置 虚拟主机域名
 * 1. 配置作用域: server { }
 *      1). server_name支持三种匹配方式, 匹配优先级从高到低依次如下:
 *          a. 完全匹配,        server_name  www.domain.com;
 *          b. 前置通配符匹配,  server_name  *domain.com;
 *          c. 后置通配符匹配,  server_name  www.domain.*;
 *          d. 正则匹配,        server_name  ~^(www\.)domain\.com$
 *
 * 2. 数据面逻辑: 
 *      1). 每个http包的host字段, 和server_name 配置的域名进行匹配, 确定是匹配了哪个虚拟主机配置(server{} 对应的 ngx_http_core_srv_conf_t)
 */
static char *
ngx_http_core_server_name(ngx_conf_t *cf/*ngx_http_core_server(srv_conf) 创建的ngx_http_conf_ctx_t*/, ngx_command_t *cmd, void *conf/*srv_conf时创建的 ngx_http_core_srv_conf_t*/)
{
    ngx_http_core_srv_conf_t *cscf = conf;
    ...
    for (i = 1; i < cf->args->nelts; i++) {
        ...
        sn = ngx_array_push(&cscf->server_names);   //!< ngx_http_core_srv_conf_t->server_names(ngx_array_t) 存储 不同server_name 对应的 ngx_http_server_name_t 结构地址
        if (sn == NULL) {
            return NGX_CONF_ERROR;
        }
        ...
        if (ngx_strcasecmp(value[i].data, (u_char *) "$hostname") == 0) {   //!< 1. Hostname: 获取nginx服务器的hostname, 并把server_name 设置为hostname
            sn->name = cf->cycle->hostname;

        } else {
            sn->name = value[i];                                            //!< 2. 完全匹配 与 首尾通配符匹配  
        }

        if (value[i].data[0] != '~') {                                      //!< 3. 正则:  对于正则匹配, server_name ~...... 即第一个字符必须是~标识
            ngx_strlow(sn->name.data, sn->name.data, sn->name.len);
            continue;                                                       //!<    不是正则, 以上逻辑已处理完毕
        }
...
#if (NGX_PCRE)                                                              //!<    正则匹配逻辑
        {
            ...
            rc.pattern = value[i];      //!< 模式串 起始地址 与 字符串长度
            rc.err.len = NGX_MAX_CONF_ERRSTR;
            rc.err.data = errstr;

            for (p = value[i].data; p < value[i].data + value[i].len; p++) {
                if (*p >= 'A' && *p <= 'Z') {   //!< 当模式串中存在大写字母, 则启用 NGX_REGEX_CASELESS/PCRE_CASELESS 标识pcre匹配匹配过程中忽略大小写
                    rc.options = NGX_REGEX_CASELESS;
                    break;
                }
            }
