...
static ngx_command_t  ngx_http_upstream_commands[] = {
    ...
    { ngx_string("server"),
      NGX_HTTP_UPS_CONF/*upstream配置上下文*/|NGX_CONF_1MORE,
...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
     ...
     * 负载均衡 后端服务器
     */
    if (u->resolved == NULL) {  //!< 通过resolved可以直接执行上游服务器地址

        /**
         * u->resolved中保存了用于与上游服务器建立连接的信息
         * 可以由开发人员在代码中设置，不设置的话，从配置文件中去获取
         */
        uscf = u->conf->upstream;

    } else {
        /**
         * upstream中直接指定了相关的服务器地址，建立连接就ok了
         */
        ...
        ctx->handler = ngx_http_upstream_resolve_handler;   //!< ngx_http_upstream_resolve_handler是域名解析后的回调函数
        ctx->data = r;
...
found:
    /* ###################################################################################### */
    /** peer.init()方法中会根据upstream的算法去选择一个服务器，来进行发送, 这个函数用来针对每个请求选择后端服务器之前做一些初始化工作
     * ngx_http_upstream_init_round_robin_peer (默认方法)
...
ngx_http_upstream(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    value = cf->args->elts;
    u.host = value[1];  //!< upstream 命令后 接着的 url 或 backend:  upstream www.baidu.com {},     upstream fastcgi_backend
    u.no_resolve = 1;

    /**
     * 新建一个ngx_http_upstream_srv_conf_t, 并加入到upngx_http_upstream_main_conf_t->upstreams 的ngx_array_t中
     */
    ...
    /** 为upstream分配一个new ngx_http_conf_ctx_t
     * new ngx_http_conf_ctx_t的main_conf 复用 全局的
     * srv_conf, loc_conf会分配新的结构
     */
     ...
     *
     * 2, 说明: 
     *  (1), 碰到upstream linuxidc, ngx_http_upstream就会新创建的ngx_http_upstream_srv_conf_t, 
     *          记录在当前upstream (url为linuxidc) 对应的ngx_http_conf_ctx_t[ngx_http_upstream_module.ctx_index]
     *
     *  (2), 此后,在解析server命令时ngx_http_upstream_server就可以获得 这个新创建的ngx_http_upstream_srv_conf_t
     *      并且, 以后每碰到一个server命令, 就会创建一个ngx_http_upstream_server_t, 并把ngx_http_upstream_server_t 插入到ngx_http_upstream_srv_conf_t->servers (ngx_array_t)中
     *
     *  (3), 碰到proxy_pass http://linuxidc,  ngx_http_proxy_pass就可根据linuxidc 找到与之对应的 ngx_http_upstream_srv_conf_t
     */
    ctx->srv_conf[ngx_http_upstream_module.ctx_index/* 0 */] = uscf;
    ...
    for (m = 0; ngx_modules[m]; m++) {
        ...
        /**
         * 对于ngx_http_proxy_module_ctx模块, 这里create_loc_conf (ngx_http_proxy_create_loc_conf) 要为这个upstream 分配 ngx_http_proxy_loc_conf_t
         */
    ...
    pcf = *cf;
    cf->ctx = ctx;
    cf->cmd_type = NGX_HTTP_UPS_CONF;   //!< 解析upstream配置上下文
...
{
    /**
     * ngx_http_upstream 中会分配一个新的ngx_http_upstream_srv_conf_t
     * 并把他记录在: ctx->srv_conf[ngx_http_upstream_module.ctx_index, 即0] = uscf;
     *
     * 而ngx_conf_handler 解析时 输入的: conf = confp[ngx_modules[i]->ctx_index, 即ngx_http_upstream_module.ctx_index, 即0];
     */
    ngx_http_upstream_srv_conf_t  *uscf = conf; //!< 获取 ngx_http_upstream  为upstream 创建的 ngx_http_upstream_srv_conf_t
    ...
    ngx_uint_t                   i;
    ngx_http_upstream_server_t  *us;            //!< 后端服务器 描述符

    if (uscf->servers == NULL) {
        uscf->servers = ngx_array_create(cf->pool, 4/*开始分配4个 ngx_http_upstream_server_t*/,
    ...
    /**
     * 命令示例: server 127.0.0.1:9015  max_fails=0;
     */
    ...
    weight = 1;         //!< 后端 默认权重为 1
    max_fails = 1;      //!< 后端 默认失败次数为 1
    fail_timeout = 10;  //!< 后端 默认最大超时时间为 10s
     ...
     * 初始化 后端服务器 描述符
     */
    us->addrs = u.addrs;                //!< ip地址
    us->naddrs = u.naddrs;
    us->weight = weight;                //!< 负载均衡 权重
    us->max_fails = max_fails;          //!< 失败次数
    us->fail_timeout = fail_timeout;    //!< 服务超时时间
...
ngx_http_upstream_add(ngx_conf_t *cf, ngx_url_t *u, ngx_uint_t flags)
{
    ...
    /**
     * 检查 现有upstream.elts中 是否和 新建的ngx_url_t *u重复
     *
     * 若重复,则直接返回 "重复的地址"
     */
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        return uscfp[i];    //!< 返回已经存在 upstream
    }
...
ngx_http_upstream_create_main_conf(ngx_conf_t *cf)
{
    ...
    if (ngx_array_init(&umcf->upstreams, cf->pool, 4,
                       sizeof(ngx_http_upstream_srv_conf_t *))  //!< 先分配 4个 ngx_http_upstream_srv_conf_t*
        != NGX_OK)
    {
        return NULL;
    }
