...
/**
 * location 的upstream 描述符
 */
typedef struct {
    ngx_http_upstream_conf_t       upstream;    //!< 后端连接的upstream描述符
...
ngx_http_proxy_handler(ngx_http_request_t *r)
{
    ...
    rc = ngx_http_read_client_request_body(r, ngx_http_upstream_init/*为当前request 初始化其upstream*/);  //!< 先读取 client端的 request line/header/body
...
{
    ngx_http_proxy_loc_conf_t *plcf = conf; //!< ngx_http_proxy_create_loc_conf 创建
    ...
    url = &value[1];    //!< proxy_pass http://url_xxx
    ...
    /** upstream url_xxx
     * 根据url_xxx, 找到 ngx_http_upstream url_xxx 新建的ngx_http_upstream_srv_conf_t
     */
