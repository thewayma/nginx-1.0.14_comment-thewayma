 ...
 * 后端服务器 描述符
 *
 * 参考: Nginx学习之十二-负载均衡-加权轮询策略剖析. http://blog.csdn.net/xiajun07061225/article/details/9318871
 */
typedef struct {
    ngx_addr_t                      *addrs;             //!< 指向存储IP地址的数组的指针，host信息(对应的是 ngx_url_t->addrs)  
    ngx_uint_t                       naddrs;            //!< 与第一个参数配合使用，数组元素个数(对应的是 ngx_url_t->naddrs)  
    ngx_uint_t                       weight;            //!< 负载均衡 权重, 默认为1

    /** 设置在fail_timeout参数设置的时间内最大失败次数
     * 如果在这个时间内，所有针对该服务器的请求都失败了，那么认为该服务器会被认为是停机了，停机时间是fail_timeout设置的时间。
     *
     * 默认情况下，不成功连接数被设置为10S
     *
     * 被设置为零则表示不进行链接数统计。
     *
     * 那些连接被认为是不成功的可以通过proxy_next_upstream, fastcgi_next_upstream，和memcached_next_upstream指令配置。
     * http_404状态不会被认为是不成功的尝试。
     */
    ...
    unsigned                         down:1;            //!< 标记服务器永久停机了；与指令ip_hash一起使用
    unsigned                         backup:1;          //!< 标记该服务器为备用服务器。当主服务器停止时，请求会被发送到它这里
...
struct ngx_http_upstream_srv_conf_s {
    ngx_http_upstream_peer_t         peer;      //!< 上游服务器集群
