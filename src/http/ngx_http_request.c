...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    if (rev->ready) {//如果已经将读事件插入到epoll中(具体见ngx_handle_read_event->ngx_epoll_add_event)，则直接调用ngx_http_init_request
...
ngx_http_init_request(ngx_event_t *rev)
{
    ...
    if (r) {
        ...
        r = ngx_pcalloc(c->pool, sizeof(ngx_http_request_t));   //!< 为当前http请求 分配对应的 http请求描述符ngx_http_request_t
        if (r == NULL) {
            ngx_http_close_connection(c);
            return;
        }
    ...
    c->data = r;                                                //!< 将请求描述符, 记录在 ngx_connection_t->data
    ...
    /**
     * c->listening 在 ngx_event_accept 初始化, 指向本地监听的 ngx_listening_t
     *
     * ngx_listening_t->servers 在ngx_http_add_addrs (<- ngx_http_init_listening <- ngx_http_optimize_servers <- ngx_http_block) 初始化
     */
    port = c->listening->servers;
    /**
     * 每个ngx_listening_t 通过 servers指针 关联到 ngx_http_port_t, 
     * for http, ngx_listening_t->servers指向 一个ngx_http_port_t 
     *
     * listen创建的ngx_http_addr_conf_t 在ngx_http_init_listening()->ngx_http_add_addrs() 与ngx_http_port_t 关联上
     */
    ...
    if (port->naddrs > 1) {
        ...
        /*
         * there are several addresses on this port and one of them is an "*:port" wildcard 
         * so getsockname() in ngx_http_server_addr() is required to determine a server address
         */
        ...
        default: /* AF_INET */
            ...
            addr_conf = &addr[i].conf;  //!< the last address is "*"
        ...
        default: /* AF_INET */
            addr = port->addrs;         //!< ngx_http_port_t *port;   port->addrs == ngx_http_in_addr_t
            addr_conf = &addr[0].conf;  //!< addr_conf == ngx_http_addr_conf_t
            break;
        }
    }
     ...
     * 建立过程见 在ngx_http_add_addrs (<- ngx_http_init_listening <- ngx_http_optimize_servers <- ngx_http_block) 初始化
     */
    r->virtual_names = addr_conf->virtual_names;

    /* the default server configuration for the address:port */
    /**
     * 获取 listening对应的 srv_conf:
     ...
     *
     * 说明:
     *  1. ngx处理每一个连接(newfd=accpet(fd), io(newfd)), 对应fd/newfd 都用一个连接描述符(ngx_connection_t) 进行抽象;
     *  2. ngx_connection_t里 读写事件(ngx_event_t, 事件都有handler回调) 与之相关联, ngx_event_t 作为epoll_clt, epoll_wait 事件驱动模型的 单元(cell);
     *  3. ngx_init_cycle 进行配置时(listen命令), 就会在main_conf(ngx_http_core_main_conf_t->port) 存放着所有listen命令配置的 端口描述符(ngx_http_conf_port_t);
     *      而ngx_http_conf_port_t->addrs 存放着 挂在此端口描述符下的 所有地址描述符(ngx_http_addr_conf_t);
     *      而ngx_http_addr_conf_t->default_server是 与这个listen socket地址(ip:port) 对应的  server{}对应的 srv_conf(ngx_http_core_srv_conf_t) )
     */
    cscf = addr_conf->default_server;

    /**
     * 确定当前request对应的 main_conf, srv_conf, loc_conf
     */
    r->main_conf = cscf->ctx->main_conf;    //!< http {} 的全局配置
    r->srv_conf = cscf->ctx->srv_conf;      //!< listen ip:port   所属的srv_conf
    ...
    if (r->header_in == NULL) {
        r->header_in = c->buffer;   //!< r->header_in 为 客户端读 buffer
    }
     ...
     */
    rev->handler(rev);  //!< 调用ngx_http_process_request_line, NGX解码状态机
}
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) {
            n = ngx_http_read_request_header(r);    //!< ngx_unix_recv, no-blocking read
...
ngx_http_process_request_headers(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) {
            ...
            n = ngx_http_read_request_header(r);    //!< ngx_unix_recv
        ...
        if (rc == NGX_OK) {
            ...
            /** headers_in_hash, 定义在 ngx_http_headers_in !!!!!!!!!!!!!!!!!!!!!!!!!
             * 在ngx_http_headers_in 匹配http相应请求头
             */
            hh = ngx_hash_find(&cmcf->headers_in_hash, h->hash,
                               h->lowcase_key, h->key.len);

            /** http头部检查回调, 定义在 ngx_http_headers_in !!!!!!!!!!!!!!!!!!!!!!!!!
             * http request header各个头部对应的 handler回调
             */
...
ngx_http_read_request_header(ngx_http_request_t *r)
{
     ...
     * last为 现收到数据的末尾位置
     *
     * ngx_http_parse_request_line 解析完后, pos指向解析完的位置上
     *
     * 若n>0, 则有可能:
     *  1. ngx_http_parse_request_line没解析完;
     *  2. 请求行已解析完了, 后面的内容是请求头, 此种场景不会继续执行ngx_http_parse_request_line, 而是跳到下一个状态中(ngx_http_process_request_headers, 解析请求头)
     * 
     */
    n = r->header_in->last - r->header_in->pos;

    if (n > 0) {
        return n;   //!< 若还有未处理(解析)的数据, 则直接返回
    }
    ...
    ngx_uint_t offset)
{
    ...
    host = h->value.data;   //!< http请求头host字段
    ...
    r->headers_in.server.data = host;   //!< 提取http request header中的host字段
...
ngx_http_process_request_header(ngx_http_request_t *r)
{
     ...
     * 1. 创建server_name哈希表
     *      ngx_http_optimize_servers 为不同server_name创建相应的哈希表(普通, 前置wildcard, 后置wildcard, 正则匹配)
     * 2. 虚拟主机匹配
     ...
     */
    if (ngx_http_find_virtual_server(r, r->headers_in.server.data/*Host字段*/,
                                     r->headers_in.server.len)
        == NGX_ERROR)
    {
...
static ngx_int_t
ngx_http_find_virtual_server(ngx_http_request_t *r, u_char *host/*http request host*/, size_t len/*host长度*/)
{
    ngx_http_core_loc_conf_t  *clcf;
    ngx_http_core_srv_conf_t  *cscf;
     ...
     * 
     * 2. 各方案哈希表布局
     *      1). 完全匹配    -   server_name www.baidu.com 
     *      2). 前缀通配符  -   server_name *baidu.com      ---> 倒序哈希表, com.baidu
     *      3). 后缀通配符  -   server_name www.baidu.*     ---> 正序哈希表, www.baidu
     *      4). 正则表达式  -   不需要哈希表
     */
