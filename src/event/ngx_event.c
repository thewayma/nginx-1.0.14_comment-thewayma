...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    /** 关于ngx_timer_resolution
     * 该配置指令允许用户减少调用gettimeofday()的次数。
     * 默认情况下，gettimeofday在每次I/O端口监听（比如epoll_wait）返回后都将被调用，而通过timer_resolution配置选项可以直接指定调用gettimeofday()函数的间隔时间。
     *
     * 具体参考: 利用timer_resolution设置减少gettimeofday调用的次数, http://www.lenky.info/archives/2011/09/69
     */
    if (ngx_timer_resolution) {             //!< 若timer_resolution设置了 epoll_wait 超时时间
        timer = NGX_TIMER_INFINITE;         //!<    则采用-1，永远阻塞, 且不执行epoll_wait返回后,不执行gettimeofday获取系统时间
        flags = 0;

    } else {
        timer = ngx_event_find_timer();     //!< 否则，用timer最近超时时间(nearest timeout timer)作为timeout参数
        flags = NGX_UPDATE_TIME;            //!< timer as epoll_wait(timeout), 目的是 处理epoll同时防止 定时器超时未被及时处理
    ...
    /** 负载均衡 互斥 锁 ---- ngx_use_accept_mutex表示是否需要通过对accept加锁来    解决惊群问题和负载均衡
     * 当nginx worker进程数>1时且配置文件中打开accept_mutex时，这个标志置为1
     *
     * 负载均衡: 当达到最大connection的7/8时，本worker不会去试图拿accept锁，也不会去处理新连接，这样其他nginx worker进程就更有机会去处理监听句柄，建立新连接了
     */
    if (ngx_use_accept_mutex) {

        /** ngx_accept_disabled>0, 放弃一次争抢accept机会
         * 见ngx_event_accept(): 
         *      ngx_accept_disabled = ngx_cycle->connection_n / 8(当前进程中所有链接对象的总数的1/8, 常量) - ngx_cycle->free_connection_n(连接池中的现有可用数);
         * ngx_accept_disabled > 0 ， 即连接池中可用数 不足 最大连接数的1/8时（已经用了7/8了）
         * ngx_accept_disabled表示此时满负荷，没必要再处理新连接了
         * 
         * 我们在nginx.conf曾经配置了每一个nginx worker进程能够处理的最大连接数，当达到最大数的7/8时，ngx_accept_disabled为正
         * 说明本nginx worker进程非常繁忙，将不再去处理新连接，这也是个简单的负载均衡
         */
        if (ngx_accept_disabled > 0) {  //大于0说明该进程接收的连接过多，放弃一次争抢accept mutex的机会
            ngx_accept_disabled--;  //!< 每放弃一次,通过-- 就增加一次几率

        } else {                        //!< 进行accpet mutex争抢机会
            /** 获得accept锁，多个worker仅有一个可以得到这把锁。
             * 1. 获得锁不是阻塞过程，都是立刻返回，获取成功的话ngx_accept_mutex_held被置为1。
             *
             * 2. 拿到锁，意味着监听句柄被放到本进程的epoll中了
             * 3. 没有拿到锁，则监听句柄会被从epoll中取出
             */
            if (ngx_trylock_accept_mutex(cycle) == NGX_ERROR) {
                return;
            }

            /** 拿到锁的话，置flag为NGX_POST_EVENTS ---- 半同步半异步(快速接收, 缓存, 延后处理)
             * 这意味着ngx_process_events函数中，任何事件都将延后处理
            ...
            if (ngx_accept_mutex_held) {
                ...
                /** 拿不到锁
                 * 也就不会处理监听的句柄，这个timer实际是传给epoll_wait的超时时间
    ...
    delta = ngx_current_msec - delta;   //!< ngx_epoll_process_events执行了多少时间

    ngx_log_debug1(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                   "timer delta: %M", delta);

    /** 处理accept后的连接事件 --- ngx_posted_accept_events暂存epoll从监听套接字接口wait到的accept事件
     * 如果ngx_posted_accept_events链表有数据，就开始accept建立新连接 
     */
    if (ngx_posted_accept_events) {
        ngx_event_process_posted(cycle, &ngx_posted_accept_events); //!< handler执行 ngx_event_accept
    }

    /** 先处理ngx_posted_accept_events队列中的事件，处理完毕后立即释放ngx_accept_mutex锁，接着再处理ngx_posted_events队列中事件。这样大大减少了ngx_accept_mutex锁占用的时间
     * 释放ngx_accept_mutex锁后, 再处理下面的EPOLLIN EPOLLOUT请求
     */
    ...
    if (ngx_posted_events) {
        if (ngx_threaded) {         //!< 通过线程方式处理
            ngx_wakeup_worker_thread(cycle);

        } else {                    //!< worker_processes同步处理, call ngx_http_init_request --- 触发http state machine(状态机处理)
...
ngx_handle_read_event(ngx_event_t *rev, ngx_uint_t flags)
{
    if (ngx_event_flags & NGX_USE_CLEAR_EVENT) {
        ...
        if (!rev->active && !rev->ready) {
            if (ngx_add_event(rev, NGX_READ_EVENT, NGX_CLEAR_EVENT) //!< ngx_epoll_add_event for epoll
                == NGX_ERROR)
            {
                return NGX_ERROR;
            }
...
ngx_event_process_init(ngx_cycle_t *cycle)
{
    ...
    if (ccf->master && ccf->worker_processes > 1 && ecf->accept_mutex) {
        ...
        ngx_accept_mutex_delay = ecf->accept_mutex_delay; // 抢ngx_accept_mutex互斥体失败后，下次再抢的间隔时间
    ...
    do {
        ...
        //链表
        c[i].data = next;   //!< 连接未使用时，data成员用于充当连接池中空闲连接链表中的next指针。当连接被使用时，data的意义由使用它的nginx模块而定
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        /* ########################################################################################################## */
        c->listening = &ls[i];  //!< 设置 listen对应的连接描述符(ngx_connection_t) 所属的 监听描述符(ngx_listening_t)
        ls[i].connection = c;   //!< 监听描述符 所属的 连接描述符(ngx_connection_t)
        /* ########################################################################################################## */
        ...
        rev->log = c->log;
        /* ########################################################################################################## */
        rev->accept = 1;    //!< 标志此read_events是 对监听的socket 的accept
        /* ########################################################################################################## */
        ...
        if (ngx_event_flags & NGX_USE_IOCP_EVENT) {
            ...
            if (ngx_add_event(rev, NGX_READ_EVENT, 0) == NGX_ERROR) {   //!< ngx_epoll_add_event, 加入到 事件驱动框架中
                return NGX_ERROR;
            }
        }
        ...
        /**
         * ngx_use_accept_mutex表示是否需要通过对accept加锁来解决惊群问题和负载均衡
         * 若采用, 则事件添加在ngx_process_events_and_timers完成
         */
        if (ngx_use_accept_mutex) {
            continue;
        }
