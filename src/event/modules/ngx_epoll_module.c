...
ngx_epoll_process_events(ngx_cycle_t *cycle, ngx_msec_t timer, ngx_uint_t flags)
{
    ...
    if (flags & NGX_UPDATE_TIME || ngx_event_timer_alarm) {
        ngx_time_update();  //!< gettimeofday 执行一次事件更新，nginx将时间缓存到一组全局变量中，方便程序高效的获取事件
    }
    //处理wait错误
    if (err) {
        if (err == NGX_EINTR) {
    ...
    for (i = 0; i < events; i++) {
        ...
        if ((revents & EPOLLIN) && rev->active/*标示事件已被激活: 已经用epoll_ctl插入到epoll中*/) {
            ...
            if (flags & NGX_POST_EVENTS) {
                queue = (ngx_event_t **) (rev->accept /*ngx_event_process_init 将listen的 read_event标示为 accept*/ ?
                               &ngx_posted_accept_events/*accept拖后队列*/ : &ngx_posted_events/*普通io拖后队列*/);

                ngx_locked_post_event(rev, queue);

            } else {
                rev->handler(rev);  //!< 将读事件结构的处理函数设置为ngx_event_accept
            }
        }
