...
typedef struct {
    ...
    // 散列表中槽(哈希表的桶数)的最大数目, 单位:个数
    ngx_uint_t        max_size;     //bucket的最大个数, 最多有几个桶
    // 散列表中一个槽的空间大小，它限制了每个散列表元素关键字的最大长度
    ngx_uint_t        bucket_size;  //每个bucket的空间大小, 单位:字节
...
typedef struct {
    ...
    ngx_array_t       keys;                 //!< 完全匹配 value存储数组 --- ngx_hash_key_t
    ...
    ngx_array_t      *keys_hash;            //!< 完全匹配 key哈希表 --- ngx_str_t

    // 用动态数组以ngx_hash_key_t 结构体保存着含有前置通配符关键字的元素生成的中间关键字
    ngx_array_t       dns_wc_head;          //!< 前缀通配符 value存储数组 --- ngx_hash_key_t
    ...
    ngx_array_t      *dns_wc_head_hash;     //!< 前缀通配符 key哈希表 --- ngx_str_t

    // 用动态数组以ngx_hash_key_t 结构体保存着含有前置通配符关键字的元素生成的中间关键字
    ngx_array_t       dns_wc_tail;          //!< 后缀通配符 value存储数组 --- ngx_hash_key_t
    /*
    ...
    */
    ngx_array_t      *dns_wc_tail_hash;     //!< 后缀通配符 key哈希表 --- ngx_str_t
