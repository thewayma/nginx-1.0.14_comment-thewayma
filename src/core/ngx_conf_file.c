...
ngx_conf_parse(ngx_conf_t *cf, ngx_str_t *filename)
{
    ...
    for ( ;; ) {
         ...
         *
         *    NGX_ERROR             there is error                              错误
         *    NGX_OK                the token terminated by ";" was found       ;结束 --- 一个配置指令结束
         *    NGX_CONF_BLOCK_START  the token terminated by "{" was found       { 开头
         *    NGX_CONF_BLOCK_DONE   the "}" was found                           } 结束
         *    NGX_CONF_FILE_DONE    the configuration file is done              解析结束
         */
        if (rc == NGX_ERROR) {
            goto done;
        }
...
ngx_conf_handler(ngx_conf_t *cf, ngx_int_t last)
{
    ...
    for (i = 0; ngx_modules[i]; i++) {
        ...
        for ( /* void */ ; cmd->name.len; cmd++) {
			...
			//最核心的地方， 获得cf->ctx = cycle->conf_ctx 配置上下文

            if (cmd->type & NGX_DIRECT_CONF) {  //!< 所有 核心模块(ngx_core_module, 指令包括: daemon, master_process等) 都具备DIRECT_CONF属性
				//我们还记得最开始ctx是包含了所有core模块的conf(create_conf回调),因此这里取出对应的模块conf.
                conf = ((void **) cf->ctx)[ngx_modules[i]->index];      //!< 1级指针, ngx_core_module 调用ngx_core_module_create_conf创建ngx_core_conf_t

            } else if (cmd->type & NGX_MAIN_CONF/*只匹配http/event命令, 注意不是NGX_HTTP_MAIN_CONF*/) { //!< 遇到了http/event命令, ngx_http_module 具备NGX_MAIN_CONF性质
                 ...
                 * 所以, 对于ngx_http_commands的 ngx_http_block而言, 要为这个指针分配内存空间, 故这里必须是 & (((void **) cf->ctx)[ngx_modules[i]->index])
                 *
                 *
                 * 注意:
                 *      遇到http命令, 就为为其分配一个ngx_http_conf_ctx_t,
                 *      并 把ngx_http_conf_ctx_t存放在 (((void **) cf->ctx)[ngx_modules[i]->index])地址上
                 *
                 *      !!!! 故, 系统只能配置一个event/http {}, 因为ngx_modules[i]->index是唯一的; 除非ngx_modules 有两个ngx_http_module !!!!
                 */
                conf = &(((void **)/* 将**** 转化为 ** */ cf->ctx)[ngx_modules[i]->index]);   //!< 2级指针, 即需要第二级 转换到具体的模块中

                /**
                 * 像http {}块, http命令具备NGX_MAIN_CONF属性;
                 * 在命令处理函数ngx_http_block, 会再分配http配置上下文ngx_http_conf_ctx_t, 
                 *      conf, 作为二级指针, 记住ngx_http_conf_ctx_t
                 */

            } else if (cf->ctx) {   //!< 对http {} 块内部 命令解析
            	//否则进入二级模块处理
                /**
                 * 1. ngx_http_block中, 给ngx_http_conf_ctx_t分配内存
                 *          ctx->main_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module);
                 *          ctx->srv_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module);
                 *          ctx->loc_conf = ngx_pcalloc(cf->pool, sizeof(void *) * ngx_http_max_module);
                 * 2. 地址转换关系
                 *      cf->ctx + cmd->conf    相当于 [main|srv|loc]_conf 指针变量的地址(&ctx->main_conf)
                 *      *(cf->ctx + cmd->conf) 相当于 [main|srv|loc]_conf 指针变量的值(ctx->main_conf)
                 */
                confp = *(void **) ((char *) cf->ctx + cmd->conf/*NGX_HTTP_MAIN_CONF_OFFSET, NGX_HTTP_SRV_CONF_OFFSET, NGX_HTTP_LOC_CONF_OFFSET*/);

                if (confp) {
                    /**
                     * 得到 ngx_http_conf_ctx_t中的 [main|srv|loc]_conf: 
