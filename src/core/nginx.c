...
ngx_uint_t          ngx_max_module;         //!< ngx_modules 模块个数
...
main(int argc, char *const *argv)
{
    ...
    for (i = 0; ngx_modules[i]; i++) {  // 这里面的ngx_modules会有非常多的模块，[ngx_core_module,ngx_errlog_module,ngx_conf_moduel]
        ngx_modules[i]->index = ngx_max_module++;   //!< 以ngx_modules的下标 作用 模块的index值
    }

    /** 对ngx_cycle结构进行初始化,这里是nginx启动核心之处！
     * 1.先后执行 ngx_modules[i] 初始化函数
     *      a. ngx_core_module_t->create_conf
     *      b. ngx_core_module_t->init_conf 
     *      c. ngx_module_t->init_module
     */
