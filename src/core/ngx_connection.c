...
ngx_create_listening(ngx_conf_t *cf, void *sockaddr, socklen_t socklen)
{
    ...
    ls = ngx_array_push(&cf->cycle->listening); //!< 存放在 ngx_cycle_t->listening数组中
    if (ls == NULL) {
        return NULL;
    }
...
/**
 * 获取一个ngx_connection_t, 并初始化相关信息(读事件, 写事件)
 */
ngx_connection_t *
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
    ...
    rev = c->read;  //!< 一个连接的 读事件
    wev = c->write; //!< 一个连接的 写事件
    ...
    rev->data = c;  //!< 读事件 所属的ngx_connection_t
    wev->data = c;  //!< 写事件 所属的ngx_connection_t
